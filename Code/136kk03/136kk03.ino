...
struct general_set {
  ...
  int temp_cal_set = 0; //калибровка датчика температуры
} general_setting;
...
#define temp_cal general_setting.temp_cal_set
...
void readencoder() { // работа с енкодером
  ...
  if (newPosition != oldPosition) { // ЕСЛИ КРУТИЛИ энкодер
    ...
    switch (menu) {
      ...
      case 9: //Настройка калибровки термометра
        if (newPosition > oldPosition) temp_cal++;
        if (newPosition < oldPosition) temp_cal--;
...
void mainscreen() { //Процедура рисования главного экрана
  ...
  switch (menu) {
    ...
    case 9: //Меню 9 - Настройка калибровки температуры
